...
function patternMeta(group) {
    ...
    const occurrences = p ? (p.count || group.investigations.length) : group.investigations.length;
    const prs = p
    ...
    html += `<dt>Seen</dt><dd>${esc(occurrences)} time(s) · ${esc(fmtDay(first))} → ${esc(fmtDay(last))}</dd>`;
    ...
    html += `</dl></div>`;
    return { html, occurrences, first, last };
}

export function renderGroup(group, patterns) {
    const n = group.investigations.length;
    const meta = patternMeta(group);
    const recurBadge = meta.occurrences > 1 ? `<span class="badge recur">×${meta.occurrences}</span>` : "";
    ...
    <div class="g-head" onclick="this.parentNode.classList.toggle('open')">
      ...
      <div>
        ...
        <div class="g-sub">${badgeCat(group.category)} ${recurBadge}
          <span>${n} investigation${n > 1 ? "s" : ""}</span>
          <span>${esc(fmtDay(meta.first))} → ${esc(fmtDay(meta.last))}</span>
        </div>
      </div>
    </div>
    <div class="g-body">
