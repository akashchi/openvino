...
"TEST_RUN"[Brief description of the merge-queue failure]
...
- **`repository`** (optional) — `owner/repo` of the analysed run. Omit to default to the current repository. If triggered by the `workflow_dispatch`, extract the `owner/repo` from the provided inputs.
