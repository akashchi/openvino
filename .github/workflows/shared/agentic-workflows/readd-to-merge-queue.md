    ...
    readd-to-merge-queue:
      ...
      inputs:
        ...
        reason:
          ...
          type: string
      steps:
<<<<<<< HEAD:.github/workflows/shared/ci-doctor-mq/readd-to-merge-queue.md
...
=======
        ...
        - name: Re-add pull request to merge queue
          shell: python
>>>>>>> ci/gha/aw-reusable-scripts-into-python:.github/workflows/shared/agentic-workflows/readd-to-merge-queue.md
          env:
...
<<<<<<< HEAD:.github/workflows/shared/ci-doctor-mq/readd-to-merge-queue.md
            ...
            # queue, `gh pr merge` with no merge strategy adds the PR to the queue
            # (or enables auto-merge if required checks are still pending) instead
            # of merging directly. `--match-head-commit` guards against the head
            # moving under us.
            if [ -n "$PR_HEAD" ]; then
              gh pr merge "$PR_NUMBER" --repo "$REPOSITORY" --match-head-commit "$PR_HEAD"
            else
...
<<<<<<< HEAD:.github/workflows/shared/ci-doctor-mq/readd-to-merge-queue.md
with no merge strategy enqueues the PR instead of merging it directly).
=======
